...
KrakenData.make_pairs(krakenPairs, populateSpread=False)

KrakenAPI.subscribe_all()
'''

KucoinAPI = KucoinAPI()
KucoinData = ExchangeData(KucoinAPI)
kucoinPairs = KucoinAPI.get_tradeable_pairs()
KucoinData.make_pairs(kucoinPairs, populateSpread=False)

pairsToUse = KucoinAPI.get_tradeable_pairs(tuple_separate=False)[100:400]
KucoinAPI.subscribe_all(pairs=pairsToUse)

#Trader = TradeExecutionModel(KrakenAPI)
GA1 = GeneticArbitrageModel(6, 400, KucoinData)
Trader = TradeExecutionModel(KucoinAPI, KucoinData)
...
while True:
    ...
    if profits:
        ...
        if GAProfit > 0 and sequence != sequence_prev:
            TraderProfit, bids, asks = Trader.verify_sequence_profit(sequence, refresh_pairs=False) # Refreshing pairs in here doesn't make snese, do it in the mainloop
        ...
        updated = [(pair[1], round(time.time() - KucoinData.Pairs[pair[1]].last_updated, 3)) for pair in sequence]
        print(f"Trader: {round(1,3)}, GA: {round(GAProfit,3)}, || {updated}  || {logged}")
